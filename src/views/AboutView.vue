<template>
    <div>
      <div class="grid grid-cols-2 gap-8 mt-20 ml-8 text-4xl">
        <p class="poppins col-start-1 row-start-5">Hello! I'm Thea.<br> A visual communicator <br>from Denmark currently studying coded design at the Danish School of Media and Journalism.</p>
      </div>
      <div class="image-container">
        <img v-for="(image, index) in images" :key="index" :src="image.src" :style="{ bottom: image.positionY + 'px', left: image.positionX + 'px' }" class="sticker absolute">
      </div>
  ...
  export default {
    data() {
      return {
        ...
        // Assign the imported PDF file to a variable
        CVTheaPDF: CVTheaPDF
      };
    },
    mounted() {
